...
//	private String BOARD_LIST_C ="select * from board where content like ? order by seq desc";
	...
	//ajax 이용 board
	   public ArrayList<BoardDTO> ajaxSearch(String writer){
	      ArrayList<BoardDTO> ajaxBoardList = new ArrayList<>();
	      //sql문 
	      String sql = "select writer, title, content, regDate from board where writer like ?";
	      ...
	      try {
	         ...
	         while(rs.next()) {
	            ...
	            dto.setWriter(rs.getString(1)); //=(rs.getString("writer");	첫번째컬럼 writer
	            dto.setTitle(rs.getString(2));
	            dto.setContent(rs.getString(3));
	            dto.setRegDate(rs.getDate(4));
	            
	            ajaxBoardList.add(dto);
	         }
