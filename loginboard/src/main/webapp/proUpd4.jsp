...
<body class="">
	...
	<div class="group_title">
		<strong class="t_green">상품 기본설정</strong>
	</div>
	<div class="data_form">
		<table class="table_form">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<div class="lineup-row">
							<input type="text" name="_sort_group" class="design number_style"
								value="100" placeholder="0" style="width: 70px"> <input
								type="hidden" name="_sort_idx" class="design" value="29.0">
							<input type="hidden" name="_idx" class="design" value="30">
				...
				<tr>
					<th class="ess">상품명</th>
					<td colspan="3"><input type="text" name="_name" class="design"
						...
						<div class="lineup-row type_process">
							<select name='pass_cate01' id="pass_cate01"
                              ...
                              <option value='519'>교환권</option></select>  -->
								<select name='pass_cate02' id="pass_cate02"
								onchange="category_select2(2);">
									<option value=''>2차 카테고리</option>
							</select>
								<select name='pass_cate03'><option value=''>3차
										카테고리</option></select>
								<a href="#none" onclick="category_add();" class="c_btn h27 blue">선택
									카테고리 추가</a>
									...
									if (_cuid) {
										$
												.ajax({
													url : "_product.inc_category_pro.php",
													type : "POST",
													data : "_cmode=delete&_code=A0344-N7288-E3075&_cuid="
															+ _cuid,
													success : function(data) {
														category_list();
													}
							...
							function category_add() {
								$
										.ajax({
											url : "_product.inc_category_pro.php",
											type : "POST",
											data : "_cmode=add&_code=A0344-N7288-E3075&pass_parent01="
													+ $(
															"select[name=pass_cate01]")
															.val()
													+ "&pass_parent02="
													+ $(
							...
							function category_select2(_idx) {
								$
										.ajax({
											...
											dataType : "json",
											type : "POST",
											data : "pass_parent03_no_required=&pass_parent01="
													+ $("[name=pass_cate01]")
															.val()
													+ "&pass_parent02="
													+ $("[name=pass_cate02]")
											...
											success : function(data) {
												if (_idx == 2) {
													...
													for (var i = 0; i < data.length; i++) {
														option_str += '<option value="' + data[i].optionValue + '" >'
																+ data[i].optionDisplay
																+ '</option>';
													}
													$(
															"select[name=pass_cate03]")
		...
		<table class="table_form">
			...
			<tbody>
				<tr>
					...
					<td>
						<div class="lineup-row">
							<input type="text" name="_buy_limit" class="design number_style"
								value="0" placeholder="0" style="width: 70px"> <span
								class="fr_tx">개</span>
						</div>
						<div class="tip_box">
	...
	<div class="data_form">
		<table class="table_form">
			...
			<tbody>
					...
					<div class="cls_coupon_type per">
						<div class="lineup-row">
							<span class="fr_tx t_orange" style="width: 40px">할인율</span> <input
								...
								class="fr_tx">최대</span> <input type="text" name="_coupon_max"
								class="design number_style" value="0" placeholder="0"
								style="width: 100px"> <span class="fr_tx">원</span>
						</div>
					</div>
				</tr>
				</div>
				...
				<div class='c_tip '>주문 시 별도로 선택해서 사용하는 쿠폰과는 별개이며 상품에 자동적용되는
					혜택입니다.</div>
				<div class='c_tip '>사용할 경우 자동아이콘(상품쿠폰 아이콘)이 상품목록과 상세에 노출되며,
					스킨에 따라 위치는 상이할 수 있습니다.</div>
				</td>
				<th>회원등급 추가혜택</th>
					...
					<td>
						<div class="mobile_tip">에디터 기능은 모바일에서 제한적일 수 있습니다.</div> <textarea
							name="_content" class="input_text SEditor"><img
								src="/upfiles/smarteditor/202212161503471433267848.jpg" title=""
								alt="딸기 밀크 마카롱"><br style="clear: both;"></textarea>
					</td>
				</tr>
		...
		<script>
					...
					function() {
						...
						$.ajax({
							...
							success : function(data) {
								...
								$('.tab_guide').find('li.hit')
										.find('.tab_menu').trigger('click');
							}
						});
					});
			// 이용내역 적용타입 선택
			$(document)
					.on(
							...
							function() {
								...
								// 변경전 값 저장
								var sv = $(
										'input[name=p_guide_type_' + idx
												+ '_save]').val();
								$('input[name=p_guide_type_' + idx + '_save]')
										.val($(this).val());

								var type = $(
										'input[name=p_guide_type_' + idx
												+ ']:checked').val();
								$('select[name=p_guide_uid_' + idx + ']')
										.hide();
			...
			$(document).on(
					...
					function() {
						...
						var type = $(
								'input[name=p_guide_type_' + sv + ']:checked')
								.val();
						...
						$('.js_chg_guide_type[data-idx=' + idx + ']:checked')
								.trigger('change');
					});
			...
			function set_guide_info(idx) {
				...
				if (oEditors.length > 0) {
					...
					oEditors.getById[id].exec('RESIZE_EDITING_AREA_BY',
							[ true ]);
				}
									...
									<div class="lineup-row">
										...
										<div class="input_file" style="width: 250px">
											...
											<div class="fileDiv">
												...
												<input type="file" name="_img_list_square" class="realFile"
													onchange="$(this).closest('.input_file').find('.fakeFileTxt').val(this.value);">
											</div>
										</div>
										...
										<label class="design"><input type="hidden"
											...
											value="auto_s_1118414415.jpg"><input type="checkbox"
											class="js_del" name="_img_list_square_DEL" value="Y">삭제</label>
									</div>
